...
function Stars(props) {
    const ref = useRef()
    const [sphere] = useState(() => random.inSphere(new Float32Array(7500), { radius: 15 }))
    ...
    return (
      <group rotation={[0, 0, Math.PI / 4]}>
        <Points ref={ref} positions={sphere} stride={3} frustumCulled={false} {...props}>
          <PointMaterial transparent color="white" size={0.03} sizeAttenuation={true} depthWrite={false} />
        </Points>
      </group>
    )
  }
...
function ScrollingText(props) {
	...
	return(
		...
		// that are vertically spaced by displacement apart.
				...
				// console.log(t);
				return <Text key={t} position={[props.position[0],props.position[1]-(displacement-props.separation),props.position[2]]} 
				color="yellow"
...
const IntroductionDiv = () =>
{
	...
	return(
		<AnimatePresence>
		<motion.div id="introduction" className="introductionDiv flex flex-col h-screen"
		ref = {ref}
		// initial = {{opacity: 0}}
		// animate = {{opacity: 1}}
		// exit = {{opacity: 0}}
		...
		<ambientLight intensity={1}/>	
			<Suspense fallback={null}>
				<ScrollingText rotation={[-Math.PI/6,0,0]} separation={0.8} text={introText} position={[0,1,-1]}/>	
